...
-- Example: instead of making individual functions to map our custom
-- data structures, we make them an instance of Functor, and implement
-- its only function: fmap
...
-- fmap id = id
        -- Mapping id over a functor should return the value unchanged

-- fmap (g . f) = (fmap g) . (fmap f)
        -- Mapping a composed function should yield the same result
        -- as mapping one function and then mapping the other
